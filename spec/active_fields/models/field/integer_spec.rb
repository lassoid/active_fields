...
RSpec.xdescribe ActiveFields::Field::Integer do
  it_behaves_like "custom_field", factory: :integer_custom_field_definition

  it "has a valid factory" do
    expect(build(:integer_custom_field_definition)).to be_valid
  end

  context "validations" do
    subject { build(:integer_custom_field_definition) }
    ...
    context "when required" do
      subject { build(:integer_custom_field_definition, :required) }
    ...
    context "with min" do
      subject { build(:integer_custom_field_definition, min: min) }
    ...
    context "with max" do
      subject { build(:integer_custom_field_definition, max: max) }
  ...
  context "callbacks" do
    ...
    describe "after_create #add_field_to_records" do
      ...
      let(:record) { build(:integer_custom_field_definition) }
      ...
      context "when default is present" do
        let(:record) { build(:integer_custom_field_definition, default: rand(0..10)) }
      ...
      context "when default is present and multiple enabled" do
        let(:record) { build(:integer_custom_field_definition, :multiple, default: rand(0..10)) }
      ...
      context "when default is nil" do
        let(:record) { build(:integer_custom_field_definition, default: nil) }
