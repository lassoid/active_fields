...
RSpec.xdescribe ActiveFields::Field::Date do
  it_behaves_like "custom_field", factory: :date_custom_field_definition

  it "has a valid factory" do
    expect(build(:date_custom_field_definition)).to be_valid
  end

  context "validations" do
    subject { build(:date_custom_field_definition) }
    ...
    context "when required" do
      subject { build(:date_custom_field_definition, :required) }
    ...
    context "with min" do
      subject { build(:date_custom_field_definition, min: min) }

      let(:min) { rand(0..10).days.from_now.to_date }

      it { is_expected.to validate_comparison_of(:max).is_greater_than_or_equal_to(min).allow_nil }
      it { is_expected.to validate_comparison_of(:default).is_greater_than_or_equal_to(min).allow_nil }
    end

    context "without min" do
      it { is_expected.not_to validate_comparison_of(:max) }
      it { is_expected.not_to validate_comparison_of(:default) }
    end

    context "with max" do
      subject { build(:date_custom_field_definition, max: max) }

      let(:max) { rand(0..10).days.from_now.to_date }

      it { is_expected.to validate_comparison_of(:default).is_less_than_or_equal_to(max).allow_nil }
    end
  ...
  context "callbacks" do
    ...
    describe "after_create #add_field_to_records" do
      ...
      let(:record) { build(:date_custom_field_definition) }
      ...
      context "when default is present" do
        let(:record) { build(:date_custom_field_definition, default: rand(0..10).days.from_now.to_date) }
      ...
      context "when default is present and multiple enabled" do
        let(:record) { build(:date_custom_field_definition, :multiple, default: rand(0..10).days.from_now.to_date) }
      ...
      context "when default is nil" do
        let(:record) { build(:date_custom_field_definition, default: nil) }
