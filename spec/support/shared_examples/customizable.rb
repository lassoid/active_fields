    ...
    describe "#where_active_fields" do
      ...
      context "with array of symbol hashes" do
        ...
        it "returns records with matching active_values" do
          expect(call_scope)
            .to include(*described_class.where(id: active_fields.map { mapping[it.id] }.inject(&:&)).to_a)
            .and exclude(*described_class.where.not(id: active_fields.map { mapping[it.id] }.inject(&:&)).to_a)
        end
      end
    ...
    describe "active_fields nested attributes autosave" do
      ...
      context "with a new active_value" do
        ...
        it "creates it" do
          ...
          active_value = record.active_values.find { it.active_field_id == active_field.id }
      ...
      context "with a persisted active_value marked for destruction" do
        ...
        it "destroys it" do
          ...
          expect(record.active_values.find { it.id == active_value.id }).to be_nil
        end
      end
    end
  end
  ...
  context "methods" do
    ...
    describe "#initialize_active_values" do
      ...
      it "doesn't save active_values that were built" do
        ...
        new_active_values =
          record.active_values.reject { existing_active_values.map(&:active_field_id).include?(it.active_field_id) }
      ...
      it "sets default values" do
        ...
        new_active_values =
          record.active_values.reject { existing_active_values.map(&:active_field_id).include?(it.active_field_id) }
        expect(new_active_values.map(&:value)).to eq(new_active_values.map { it.active_field.default_value })
      end
