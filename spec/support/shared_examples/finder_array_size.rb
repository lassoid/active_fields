...
RSpec.shared_examples "finder_array_size" do
  context "with size_eq op" do
    ...
    context "when value is a number" do
      let(:value) do
        size = records.map { |r| r.value.size }.uniq.sample
        [size, size.to_s].sample
      end

      it "returns only records with elements count equal to the value" do
        expect(perform_search)
          .to include(*records.select { |r| r.value.size == value.to_i })
          .and exclude(*records.reject { |r| r.value.size == value.to_i })
      end
    end
  ...
  context "with size_not_eq op" do
    ...
    context "when value is a number" do
      ...
      it "returns only records with elements count not equal to the value" do
        expect(perform_search)
          .to include(*records.reject { |r| r.value.size == value.to_i })
          .and exclude(*records.select { |r| r.value.size == value.to_i })
      end
    end
  ...
  context "with size_gt op" do
    ...
    context "when value is a number" do
      let(:value) do
        size = (records.map { |r| r.value.size }.uniq - [records.map { |r| r.value.size }.max]).sample
        [size, size.to_s].sample
      end
  ...
  context "with size_gteq op" do
    ...
    context "when value is a number" do
      let(:value) do
        size = (records.map { |r| r.value.size }.uniq - [records.map { |r| r.value.size }.min]).sample
        [size, size.to_s].sample
      end
