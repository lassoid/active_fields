  ...
  describe "#serialize" do
    ...
    context "with UTC timezone" do
      context "when array of dates" do
        ...
        it { is_expected.to eq(value.map { _1.to_time(:utc).iso8601(max_precision) }) }
      end

      context "when array of date strings" do
        let(:value) { [random_date.iso8601, random_date.iso8601] }

        it { is_expected.to eq(value.map { Date.parse(_1).to_time(:utc).iso8601(max_precision) }) }
      end

      context "when array of datetimes" do
        let(:value) { [random_datetime, random_datetime] }

        it { is_expected.to eq(value.map { _1.utc.iso8601(max_precision) }) }
      end
  ...
  describe "#deserialize" do
    ...
    context "with UTC timezone" do
      ...
      context "when array of datetimes" do
        ...
        it { is_expected.to eq(value.map { apply_datetime_precision(_1.in_time_zone, max_precision) }) }
      end
    ...
    context "with precision" do
      ...
      context "when array of datetime strings" do
        ...
        it "casts elements to time objects with provided precision" do
          expect(call_method).to eq(
            value.map { apply_datetime_precision(Time.zone.parse(_1).in_time_zone, args[:precision]) },
          )
        end
      end
    end
