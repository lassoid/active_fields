    ...
    context "with eq op" do
      ...
      context "when value is true" do
        ...
        it "returns only records with truthy value" do
          expect(perform_search)
            .to include(*records.select { it.value.is_a?(TrueClass) })
            .and exclude(*records.reject { it.value.is_a?(TrueClass) })
        end
      end
      ...
      context "when value is false" do
        ...
        it "returns only records with falsy value" do
          expect(perform_search)
            .to include(*records.select { it.value.is_a?(FalseClass) })
            .and exclude(*records.reject { it.value.is_a?(FalseClass) })
        end
      end
    ...
    context "with not_eq op" do
      ...
      context "when value is true" do
        ...
        it "returns only records with not truthy value" do
          expect(perform_search)
            .to include(*records.reject { it.value.is_a?(TrueClass) })
            .and exclude(*records.select { it.value.is_a?(TrueClass) })
        end
      end
      ...
      context "when value is false" do
        ...
        it "returns only records with not falsy value" do
          expect(perform_search)
            .to include(*records.reject { it.value.is_a?(FalseClass) })
            .and exclude(*records.select { it.value.is_a?(FalseClass) })
        end
      end
