...
6. Use the `where_active_values` query method to filter records and add a search form in _Customizables_ index actions.
...
- Permitted parameters (can contain either an array of hashes or a hash of hashes).
    ...
    ```ruby
    Post.where_active_values(permitted_params)
    ```
